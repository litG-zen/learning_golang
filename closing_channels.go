...
func main() {
	...
	// This sends JOB_SIZE jobs to the worker over the `jobs`
	// channel, then closes it.
	go func() {
		// Interesting thing to note here is that, when buffered-channel is involved in goroutines
		// with one goroutine sending and another receiving
		// the execution is always sender first and then receiver.
		for j := 1; j <= cap(jobs); j++ {
			jobs <- j
